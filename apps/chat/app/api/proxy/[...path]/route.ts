...
// For development: Disable SSL certificate verification for ELB URLs
// This is needed because ELB URLs don't match the SSL certificate (cert is for raweval.com)
if (
  process.env.NODE_ENV === 'development' &&
  API_BASE_URL.includes('elb.amazonaws.com') &&
  API_BASE_URL.startsWith('https://')
) {
  // Disable SSL verification for development only (handles certificate mismatch)
  // This is safe because we're only proxying to known backend URLs
  process.env.NODE_TLS_REJECT_UNAUTHORIZED = '0';
  console.warn(
    '⚠️  SSL certificate verification disabled for development (ELB URL certificate mismatch)'
  );
}
